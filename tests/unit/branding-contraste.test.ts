...
describe("extrairRegua — os pares saem do globals.css, nunca de lista à mão", () => {
  it("acha os dois temas, a rampa do produto e os neutros", () => {
    ...
    expect(REGUA.rampaDoProduto[6]).toBe("#7b3fa4");
  ...
  it("reproduz as razões medidas à mão no design system", () => {
    ...
    expect(razao("--color-accent", "--color-bg")).toBeCloseTo(6.53, 2);
    expect(razao(":focus-visible/outline", "--color-bg")).toBeCloseTo(4.46, 2);
    expect(razao(":focus-visible/outline", "--color-surface-elevated")).toBeCloseTo(4.18, 2);
  });
...
describe("reconciliação — quem se move são as NOSSAS semânticas", () => {
  it("uma marca Sage dispara a reconciliação sem contaminar a semântica do produto", () => {
  ...
  it("toda semântica movida folga do accent e continua dentro do orçamento de rotação", () => {
    ...
    // Guarda de vacuidade do run inteiro, recalibrada para a régua IMOBI.
...
describe("marca acromática — o accent do produto permanece", () => {
  ...
  it("o accent que permanece é cromático e separável do neutro do mesmo grau", () => {
    ...
    // Números exatos da régua IMOBI; vigiam deriva silenciosa dos neutros e do accent.
    expect(separacaoDoNeutro(REGUA.claro, marca.claro.grauDoAccent, marca.claro.accent)).toBeCloseTo(0.1453, 4);
    expect(separacaoDoNeutro(REGUA.escuro, marca.escuro.grauDoAccent, marca.escuro.accent)).toBeCloseTo(0.2550, 4);
