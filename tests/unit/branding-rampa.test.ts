...
describe("rampaDeSemente — catraca de calibração contra o design system", () => {
  ...
  it("lê 11 stops distintos do globals.css (guarda de vacuidade)", () => {
    ...
    expect(esperados[K]).toBe("#7b3fa4");
  });

  it("deriva 11 stops distintos preservando a semente IMOBI", () => {
    const derivada = rampaDeSemente("#7b3fa4");
    expect(derivada).toHaveLength(11);
    expect(new Set(derivada).size).toBe(11);
    expect(derivada[K]).toBe("#7b3fa4");
  });
