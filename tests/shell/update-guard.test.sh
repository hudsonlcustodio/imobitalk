...
NS="$(bash -c 'source "$1"; printf "%s" "$IMG_NS"' _ "$(cd "$(dirname "${BASH_SOURCE[0]}")/../../hostgator-setup-kit" && pwd)/_common.sh")"
...
check ".env aponta para a imagem da versão instalada" grep -q "^APP_IMAGE=${NS}/imobi-crm:1.1.0$" .env
...
check "o worker é pinado na MESMA versão do app" \
  grep -q "^WORKER_IMAGE=${NS}/imobi-worker:1.1.0$" .env
check "o scheduler é pinado na MESMA versão do app" \
  grep -q "^SCHEDULER_IMAGE=${NS}/imobi-scheduler:1.1.0$" .env
...
check "a imagem do .env continua intacta" grep -q "^APP_IMAGE=${NS}/imobi-crm:latest$" .env
...
check "NÃO reescreveu a imagem do .env" grep -q "^APP_IMAGE=${NS}/imobi-crm:latest$" .env
...
pin_caso "app pinado + worker/scheduler AUSENTES → acusa os dois" \
  "APP_IMAGE=${NS}/imobi-crm:1.3.0" "worker scheduler"
pin_caso "app pinado + worker em canal móvel → acusa" \
  "APP_IMAGE=${NS}/imobi-crm:1.3.0
WORKER_IMAGE=${NS}/imobi-worker:stable
SCHEDULER_IMAGE=${NS}/imobi-scheduler:1.3.0" "worker"
pin_caso "as três na mesma versão → silêncio" \
  "APP_IMAGE=${NS}/imobi-crm:1.3.0
WORKER_IMAGE=${NS}/imobi-worker:1.3.0
SCHEDULER_IMAGE=${NS}/imobi-scheduler:1.3.0" ""
...
case "$*" in
  ...
  # resolução o dublê devolvia a string `${NS}/imobi-worker:stable` — uma
...
R="$( printf "APP_IMAGE=${NS}/imobi-crm:1.3.0\n" > "$PIN_DIR/.env"
