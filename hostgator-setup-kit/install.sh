#!/usr/bin/env bash
#
# IMOBI CRM — instalador self-host para VPS (HostGator).
#
...
if [ -f "$COMPOSE" ]; then
  ...
  [ -n "$REPO_URL" ] || { c_red "✖ IMOBI_REPO_URL/REPO_URL não configurada; execute o kit dentro do fork ou informe a URL do repositório IMOBI."; exit 1; }
...
e, se for mesmo um Traefik, ponha REVERSE_PROXY=traefik no .env e rode de novo."
  ...
  *)
    ...
    if [ -n "$dono_arvore" ] && [ "$dono_projeto" = "$proj_atual" ] && [ "$dono_arvore" != "$_minha_arvore" ]; then
      c_red "✖ Já existe um IMOBI CRM NO AR nesta VPS, instalado em ${dono_arvore}."
      printf '\n%s\n'   "  Esta pasta (${_minha_arvore}) é outra cópia do repo. As duas se chamam"
      printf '%s\n'     "  IMOBI CRM, então o Docker dá às duas o MESMO nome de projeto"
      ...
      die "Instalação interrompida para não derrubar o IMOBI CRM que está no ar em ${dono_arvore}."
    fi
    ...
    printf '\n%s\n'   "  1. Já é outro IMOBI CRM neste servidor? Então use aquele — entre na"
...
if [ -z "${NEXT_PUBLIC_SUPABASE_URL:-}" ] && [ -n "${SUPABASE_ACCESS_TOKEN:-}" ]; then
  ...
  _sb_out="$(bash "$KIT_DIR/supabase-provision.sh" "${APP_NAME:-IMOBI CRM}" "${SUPABASE_REGION:-sa-east-1}")" \
...
FIELDS=(
  ...
  "APP_NAME|Nome que aparece na interface (Enter para o padrão)|IMOBI CRM|||"
