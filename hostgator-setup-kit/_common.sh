...
enter_project() {
  if [ -f "$COMPOSE" ]; then :;
  elif [ -f "imobi-crm/$COMPOSE" ]; then cd imobi-crm;
elif [ -f "deskcommcrm/$COMPOSE" ]; then cd deskcommcrm; # legado upstream
...
ghcr_status() {
  ...
  tok="$(curl -fsS --max-time 6 \
          "https://ghcr.io/token?scope=repository:${IMG_NS#ghcr.io/}/${img}:pull&service=ghcr.io" 2>/dev/null \
        | sed -n 's/.*"token":"\([^"]*\)".*/\1/p')" || true
  if [ -z "$tok" ]; then printf '000'; return 0; fi
  curl -s -o /dev/null --max-time 6 -w '%{http_code}' \
    -H "Authorization: Bearer $tok" \
    -H 'Accept: application/vnd.oci.image.index.v1+json,application/vnd.docker.distribution.manifest.list.v2+json,application/vnd.docker.distribution.manifest.v2+json' \
    "https://ghcr.io/v2/${IMG_NS#ghcr.io/}/${img}/manifests/${tag}" 2>/dev/null || printf '000'
}
...
trio_publicado() {
  local tag="$1" i
  for i in imobi-crm imobi-worker imobi-scheduler; do
...
completar_pin_ausente() {  # completar_pin_ausente [envfile]
  ...
  for par in "WORKER_IMAGE:worker:imobi-worker" "SCHEDULER_IMAGE:scheduler:imobi-scheduler"; do
