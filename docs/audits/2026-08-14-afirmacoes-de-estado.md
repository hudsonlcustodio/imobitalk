...
```bash
gh api repos/hudsonlcustodio/imobitalk/branches/main/protection --jq '.required_status_checks.contexts|join(", ")' ; sed -n '10,17p' .github/workflows/e2e.yml
```
...
**Sugestão:** É check obrigatório — confirme antes de citar: `gh api repos/hudsonlcustodio/imobitalk/branches/main/protection --jq '.required_status_checks.contexts'`. (A data de ativação não é auditável pelo repo; e o cabeçalho de `.github/workflows/e2e.yml` ainda diz que o `e2e` NÃO é obrigatório — esse comentário está podre e contradiz a branch protection.)
...
```bash
gh issue view 63 --repo hudsonlcustodio/imobitalk --json number,title,state
```
...
```bash
ls tests/e2e/*.spec.ts | wc -l ; gh api repos/hudsonlcustodio/imobitalk/branches/main/protection --jq '.required_status_checks.contexts|join(", ")'
```
...
```bash
gh api repos/hudsonlcustodio/imobitalk/branches/main/protection --jq '.required_status_checks.contexts|join(", ")' ; grep -n 'run: pnpm' .github/workflows/ci.yml .github/workflows/perf.yml
```
...
```bash
gh api repos/hudsonlcustodio/imobitalk/branches/main/protection --jq '.required_status_checks.contexts|join(", ")' ; sed -n '70,79p' CONTRIBUTING.md
```
...
```bash
gh api repos/hudsonlcustodio/imobitalk/branches/main/protection --jq '.required_status_checks.contexts|join(", ")'
```
...
**Sugestão:** O CI reprova serviço `build:`-only, instalação em tag móvel **e imagem que não constrói** — as três coisas bloqueiam merge. Confira a lista viva com `gh api repos/hudsonlcustodio/imobitalk/branches/main/protection --jq '.required_status_checks.contexts'`;
...
```bash
gh api repos/hudsonlcustodio/imobitalk/branches/main/protection --jq '.required_status_checks.contexts|join(", ")' ; sed -n '356,358p' README.en.md
```
...
**Sugestão:** (pt-br: dizer que a linha é só o que roda local.) Texto pronto: "That line is everything you can run on **your own machine**, on purpose: running half of them and discovering the rest as a red surprise after hours of waiting is the worst first experience this repository knows how to deliver. It covers `verify`, `build-and-size` and `invariants`; the other two required checks (`e2e` and `imagens-ok`) only run in CI. The live list comes from `gh api repos/hudsonlcustodio/imobitalk/branches/main/protection --jq '.required_status_checks.contexts|join(\", \")'`."
...
```bash
gh api repos/hudsonlcustodio/imobitalk/branches/main/protection --jq '.required_status_checks.contexts|join(", ")' ; sed -n '357,358p' README.es.md
```
...
```bash
gh api repos/hudsonlcustodio/imobitalk/branches/main/protection --jq '.required_status_checks.contexts|join(", ")'; grep -n 'pnpm typecheck && pnpm lint' README.md
```
...
```bash
gh api repos/hudsonlcustodio/imobitalk/security-advisories --jq '.[] | "\(.ghsa_id) criado=\(.created_at) estado=\(.state)"'
```
...
**Sugestão:** | CI (mecânico) | `imagens-ok` em `publish-image.yml` | imagem quebrada reprova o merge — é required check da `main`. Confira na fonte antes de confiar nesta linha: `gh api repos/hudsonlcustodio/imobitalk/branches/main/protection --jq '.required_status_checks.contexts'` |
...
```bash
docker buildx imagetools inspect ghcr.io/hudsonlcustodio/imobitalk:stable --format '{{.Manifest.Digest}}' ; docker buildx imagetools inspect ghcr.io/hudsonlcustodio/imobitalk:1.3.0 --format '{{.Manifest.Digest}}'
```
...
```bash
gh api repos/hudsonlcustodio/imobitalk/branches/main/protection --jq '.required_status_checks.contexts|join(", ")' ; ghcr_status deskcommcrm stable
```
...
**Sugestão:** | PR de contribuidor | `imagens-ok` rodava e informava; não bloqueava. **Desde 2026-08-14 bloqueia** — é required check da `main` (`gh api repos/hudsonlcustodio/imobitalk/branches/main/protection --jq '.required_status_checks.contexts\|join(", ")'`) |
...
docker-compose.prod.yml:35 image: ${APP_IMAGE:-ghcr.io/hudsonlcustodio/imobitalk:stable} (idem worker:92 e scheduler:196)
...
```bash
docker buildx imagetools inspect ghcr.io/hudsonlcustodio/imobitalk-worker:stable 2>&1 | awk '/^Digest:/{print $2; exit}'
```
...
```bash
docker buildx imagetools inspect ghcr.io/hudsonlcustodio/imobitalk-worker:stable 2>&1 | awk '/^Digest:/{print $2;exit}'; docker buildx imagetools inspect ghcr.io/hudsonlcustodio/imobitalk-worker:1.3.0 2>&1 | awk '/^Digest:/{print $2;exit}'; git rev-parse v1.3.0^{commit}
```
...
```bash
ls tests/e2e/*.spec.ts | wc -l; awk '/SPECS_PARTE_1:/,/FORA_DO_CI:/' .github/workflows/e2e.yml | grep -v '^ *#' | grep -oE '[a-z0-9-]+\.spec\.ts' | wc -l; gh api repos/hudsonlcustodio/imobitalk/branches/main/protection --jq '.required_status_checks.contexts|join(", ")'
```
...
**Sugestão:** Apagar a pergunta 7. Está respondida: são cinco checks obrigatórios (`verify, build-and-size, invariants, e2e, imagens-ok`), e o gate de RLS (`invariants`) é bloqueante. A régua fica no lugar da resposta: `gh api repos/hudsonlcustodio/imobitalk/branches/main/protection --jq '.required_status_checks.contexts|join(", ")'`.
...
**Sugestão:** Acrescentar, entre a linha de `[Não lançado]` e a de `[1.2.1]`:  [1.3.0]: https://github.com/hudsonlcustodio/imobitalk/compare/v1.2.1...v1.3.0
...
> Imagem publicada em `ghcr.io/hudsonlcustodio/imobitalk` — a VPS não compila nada.
...
```
Em v1.0.0: `app` (linha 19) tem `image: ${APP_IMAGE:-ghcr.io/hudsonlcustodio/imobitalk:latest}`; `worker` (linha 42) tem **`build:`** e NENHUM `image:`; `scheduler` (linha 112) é `alpine:3.20` com `apk add` no `command:`. Ou seja, na própria v1.0.0 a VPS compilava o worker. O mesmo arquivo admite isso na linha 22: "O worker … era compilado dentro do seu servidor no dia da instalação".
```

**Sugestão:** - Imagem publicada em `ghcr.io/hudsonlcustodio/imobitalk` — a VPS não compila **o app**. (O worker ainda era compilado no seu servidor nesta versão; isso só foi corrigido na 1.3.0.)
...
**Sugestão:** - CI com portões obrigatórios na `main` — na 1.0.0 eram dois, `verify` (typecheck, lint, testes unitários) e `invariants`. A lista em vigor hoje é sempre esta, medida na fonte: `gh api repos/hudsonlcustodio/imobitalk/branches/main/protection --jq '.required_status_checks.contexts|join(", ")'`.
...
> [Não lançado]: https://github.com/hudsonlcustodio/imobitalk/compare/v1.2.1...HEAD
...
**Sugestão:** [Não lançado]: https://github.com/hudsonlcustodio/imobitalk/compare/v1.3.0...HEAD
...
```bash
gh api repos/hudsonlcustodio/imobitalk/branches/main/protection --jq '{contexts:.required_status_checks.contexts, strict:.required_status_checks.strict, reviews:.required_pull_request_reviews}'
```
...
```bash
grep -n 'vps-webhook-outbound-ssrf' .github/workflows/e2e.yml; gh api repos/hudsonlcustodio/imobitalk/branches/main/protection --jq '.required_status_checks.contexts|join(", ")'
```
...
```bash
gh api repos/hudsonlcustodio/imobitalk/branches/main/protection --jq '.required_status_checks.contexts|join(", ")'; sed -n '155,178p' .github/workflows/publish-image.yml
```
