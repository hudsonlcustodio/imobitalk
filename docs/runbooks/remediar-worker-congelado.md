...
|---|---|---|
| app | `ghcr.io/hudsonlcustodio/imobitalk:1.2.1` (registry) | 2026-08-12 |
...
printf 'WORKER_IMAGE=ghcr.io/hudsonlcustodio/imobitalk-worker:<a-mesma-versão>\n' >> .env
...
.env depois da 1ª execução:
  APP_IMAGE=ghcr.io/hudsonlcustodio/imobitalk:1.3.0     ← pinado
...
`ghcr.io/hudsonlcustodio/imobitalk-worker:stable`**.

O resultado é uma imagem local **com nome de registry** — que parece publicada e não é:

```
revision: []                                          ← vazio: não veio do CI
source:   [https://github.com/hudsonlcustodio/imobitalk] ← veio do LABEL do Dockerfile
