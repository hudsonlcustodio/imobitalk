...
worker:
  image: ${WORKER_IMAGE:-ghcr.io/hudsonlcustodio/imobitalk-worker:stable}
  build: { context: ., dockerfile: Dockerfile.worker }
```
...
> `APP_VERSION` — medido: `docker run --rm ghcr.io/hudsonlcustodio/imobitalk:1.2.1 node -e
...
[ ] 8. A imagem reporta a versão certa:
       docker run --rm ghcr.io/hudsonlcustodio/imobitalk:X.Y.Z \
...
|---|---|---|
| CI (mecânico) | `imagens-ok` em `publish-image.yml` | imagem quebrada **reprova o merge** — é required check da `main`. Meça antes de confiar: `gh api repos/hudsonlcustodio/imobitalk/branches/main/protection --jq '.required_status_checks.contexts'` |
...
`ghcr.io/hudsonlcustodio/imobitalk`, que é o que o CI publica e o que está gravado no `.env` de
