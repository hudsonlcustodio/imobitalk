...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    title: {
      default: `${name} — CRM imobiliário`,
      template: `%s · ${name}`,
    },
    description:
      "CRM imobiliário para organizar leads, atendimento, oportunidades, imóveis, visitas e operação comercial em uma única experiência.",
...
const THEME_INIT_SCRIPT = `(function(){try{var s=localStorage.getItem('imobi-theme');var d=window.matchMedia('(prefers-color-scheme: dark)').matches;var r=(s==='dark'||s==='light')?s:((s==='system'||!s)&&d?'dark':'light');document.documentElement.setAttribute('data-theme',r);}catch(e){document.documentElement.setAttribute('data-theme','light');}})();`;
