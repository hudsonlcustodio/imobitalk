#!/usr/bin/env bash
# Backup do Postgres (Supabase) do IMOBI CRM — schema public completo
# (CRM + harness do agente). Roda no host ou num cron da VPS:
#   0 3 * * * /path/repo/scripts/backup-db.sh /var/backups/imobi
...
find "$DIR" -name 'imobi-*.dump' -mtime +"$RETENTION_DAYS" -delete
