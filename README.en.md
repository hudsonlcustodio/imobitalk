...
Open an [issue](https://github.com/hudsonlcustodio/imobitalk/issues/new/choose) — the template asks for what we need (environment, `/api/v1/health`, steps). Running `bash hostgator-setup-kit/healthcheck.sh` and pasting the output helps a lot.

For **security vulnerabilities**, **do NOT open a public issue** — use [private vulnerability reporting](https://github.com/hudsonlcustodio/imobitalk/security/advisories/new). Details in [`SECURITY.md`](SECURITY.md).
